...
namespace DbgHelp.MinidumpFiles
{
    public class MiniDumpFile : IDisposable
    {
        ...
        /// <returns></returns>
        public static MiniDumpFile OpenExisting(FileStream SFfileStream, StorageFile file)
        {
            ...
            FileStream fileStream = SFfileStream;
            
            try
            {
                minidumpMappedFile = MemoryMappedFile.CreateFromFile(fileStream, Path.GetFileName(file.Path), 0, MemoryMappedFileAccess.Read, HandleInheritability.None, false);
